...
def factories():
    # Subconjunto de candidatos fortes/rápidos para a seleção nested.
    # O campeonato amplo continua testando mais famílias separadamente.
    return {
        "ridge":lambda:make_pipeline(StandardScaler(),Ridge(alpha=2.0)),
        "poisson":lambda:make_pipeline(StandardScaler(),PoissonRegressor(alpha=.2,max_iter=250)),
        "extra_trees":lambda:ExtraTreesRegressor(n_estimators=40,min_samples_leaf=5,random_state=42,n_jobs=-1),
        "hist_gradient_boosting":lambda:HistGradientBoostingRegressor(max_iter=40,max_leaf_nodes=12,l2_regularization=1.0,random_state=42),
    }
...
def fit_predict(name,train,test,s):
    ...
    ycol=f"target_{s}"; feats=feature_cols(s); ytr=train[ycol].fillna(0).to_numpy(float)
...
def choose_model(history,s,current_round):
    prior=history[history.rodada<current_round].copy(); rounds=sorted(prior.rodada.unique())
    ...
    for name in ["media3","ewma",*factories().keys()]:
        try:scores[name]=float(mean_absolute_error(y,fit_predict(name,inner_train,val,s)))
...
def main():
    ...
    for rodada in sorted(int(r) for r in df.rodada.unique() if int(r)>=START_ROUND):
        ...
        for pos in POSITIONS:
            mask=current.posicao.eq(pos); test=current[mask]; hist=previous[previous.posicao.eq(pos)]
            if test.empty:continue
            local_score=np.zeros(len(test),float)
            for s,w in SCOUT_WEIGHTS.items():
                if f"target_{s}" not in df.columns:continue
                winner,inner_scores=choose_model(hist,s,rodada); champion_counts[winner]=champion_counts.get(winner,0)+1
                if len(hist)<MIN_TRAIN:winner="ewma"
                try:pred=fit_predict(winner,hist,test,s)
        ...
        for i,row in current.iterrows():
            j=current.index.get_loc(i)
            predictions.append({"rodada":rodada,"atleta_id":int(row.atleta_id),"posicao":row.posicao,"real":float(row.target_pontos),"v3s_nested":float(score[j]),"direta_ewma":float(row.pontos_ewma)})
    ...
    payload={"gerado_em":datetime.now(timezone.utc).isoformat(),"protocolo":"nested walk-forward: em cada R, seleção usa apenas duas rodadas anteriores como validação e treino encerrado antes delas; vencedor é retreinado só em histórico < R","inicio_rodada":START_ROUND,"linhas":int(len(pred)),"rodadas":sorted(pred.rodada.astype(int).unique().tolist()),"scouts":scouts,"modelos_selecao":["media3","ewma",*factories().keys()],"nota_modelos":"Nested usa subconjunto eficiente; campeonato amplo testa famílias adicionais sem contaminar a seleção temporal.","geral":geral,"por_posicao":por_pos,"por_rodada":por_rodada,"vitorias_rodada_v3s":wins,"empates_rodada":ties,"vitorias_rodada_direta":len(por_rodada)-wins-ties,"selecoes_modelos":dict(sorted(champion_counts.items(),key=lambda kv:(-kv[1],kv[0]))),"log_selecao":selection_log}
