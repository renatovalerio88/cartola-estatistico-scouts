...
def evaluate(df,s,pos):
    ...
    if ycol not in sub.columns or len(sub)<MIN_ROWS:return None
    ...
    ranking=sorted(metrics,key=lambda n:(metrics[n]["mae"],metrics[n]["rmse"])); return {"scout":s,"posicao":pos,"ranking":ranking,"metricas":metrics,"rodadas_teste":sorted(set(rounds_used)),"eventos_reais":round(float(actual.sum()),3)}
...
def main():
    ...
    payload={"gerado_em":datetime.now(timezone.utc).isoformat(),"protocolo":"walk-forward em 4 blocos temporais; cada bloco é previsto por modelo treinado exclusivamente em rodadas anteriores","min_linhas_treino":MIN_ROWS,"modelos":["media3","ewma",*models().keys()],"scouts_testados":SCOUTS,"competicoes_validas":len(results),"vitorias_modelos":dict(sorted(wins.items(),key=lambda x:(-x[1],x[0]))),"resultados":results}
    OUT.parent.mkdir(parents=True,exist_ok=True); OUT.write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding="utf-8"); print(f"Campeonato ampliado concluído: {len(results)} scout×posição válidos.")
